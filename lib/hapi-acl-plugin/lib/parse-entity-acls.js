...
const {concat, uniq, pipe, isNil, not, filter, isEmpty, contains, map, assoc, and} = require('ramda');
...
const changeTenantsOfRoles = (fromTenant, toTenant, roles) =>
    map(r => {
...
module.exports = (atrixACL, acls, rawRoles) => {
    ...
    acls.forEach(acl => {
        ...
        const roleDefinitions = buildRolesDefinitionArrayFromRoles({
            atrixACL,
            roles: rawRolesToMap,
            source: `acl:${acl.resId}`,
        });
