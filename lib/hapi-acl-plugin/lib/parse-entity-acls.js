...
const {
	concat, uniq, pipe, isNil, not, filter, isEmpty, contains, map, assoc,
} = require('ramda');
...
module.exports = (atrixACL, acls, rawRoles) => {
	...
	acls.forEach((acl) => {
		...
		const roleDefinitions = buildRolesDefinitionArrayFromRoles({ atrixACL, roles: rawRolesToMap });
		let newRoles = concat(changeTenantsOfRoles(acl.acl.tenantId, acl.entityTenantId, roleDefinitions), roles);
		if (isNotNil(acl.acl.roles)) {
