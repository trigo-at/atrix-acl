...
describe('buildRolesDefinitionsArrayfromRoles', () => {
    ...
    it('parses roles', () => {
        expect(
            buildRolesDefinnitionArrayFromRoles({atrixACL, roles: ['ak:editor', 'voegb:admin'], source: 'token'})
        ).to.eql([
    ...
    it('filters doubles', () => {
        expect(
            buildRolesDefinnitionArrayFromRoles({
                atrixACL,
                roles: ['ak:editor', 'ak:editor', 'voegb:admin'],
                source: 'token',
            })
        ).to.eql([
            {tenant: 'ak', role: 'editor', global: false, source: 'token'},
    ...
    it('handles global roles', () => {
        expect(
            buildRolesDefinnitionArrayFromRoles({
                atrixACL,
                roles: ['ak:editor', 'super-admin', 'voegb:admin'],
                source: 'token',
            })
        ).to.eql([
            {tenant: 'ak', role: 'editor', global: false, source: 'token'},
