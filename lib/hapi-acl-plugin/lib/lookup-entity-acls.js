...
module.exports = ({path, method, log, atrixACL}) => {
    const matchPath = atrixACL.fixPath(path);

    const matchingCfg = atrixACL.ACL.rules.find(r => r.path && methodFilter(r, method) && pathFilter(r, matchPath));
    ...
    const effectiveEntityAcls = atrixACL.ACL.entityACLs.filter(
        a => a.entity === matchingCfg.entity && a.id === params[matchingCfg.idParam]
    );
