...
module.exports = (req, atrixACL) => {
	...
	const matchingCfg = find(
		r => and(methodFilter(r, req.method), pathFilter(r, matchPath)),
		atrixACL.ACL.rules
	);
	...
	const effectiveEntityAcl = defaultTo(
		null,
		find(
			a =>
				a.entity === matchingCfg.entity &&
				a.id === params[matchingCfg.idParam],
			atrixACL.ACL.entityACLs
		)
	);
