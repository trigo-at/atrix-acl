...
const {isNil, not, pipe, concat, uniq, contains, clone, map} = require('ramda');
...
module.exports = (atrixACL, req, links, providedRoles, userId, tenantId) => {
    const filtered = Object.keys(links).reduce((ret, transition) => {
        ...
        const filteredRoles = tenantId ? roles.filter(r => contains(r.tenant, effectiveTenantIds) || r.global) : roles;
