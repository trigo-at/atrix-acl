...
module.exports = (obj, filter, shouldFilterHatrLinks, atrixACL, req) => {
	...
	const whenClosure = (path, value, root) => (filter.when ? filter.when({ path, root, obj: value, value, filter, req }) : true);
	...
	filterNode(0, obj, obj, '', trie, filter.value, whenClosure, filterHatrClosure);
...
function filterNode(level, root, node, path, filterTrie, value, when, filterHatr, contextTenantId) {
	...
	if (nodesToProcess.length > 0) {
		processBlacklist(nodesToProcess, childNodesToTraverse, node, path, value, when, root);
	}

	if (hasWildcard) {
		const retryFilter = filterTrie['*'];
		debug(' next* -> ', ' ', node, retryFilter);
		filterNode(level, root, node, path, retryFilter, value, when, filterHatr, contextTenantId);
	}
	...
	childNodesToTraverse.forEach(attr => {
		...
		filterNode(level + 1, nextRoot, nextNode, constructPath(path, attr), nextTrie, value, when, filterHatr, contextTenantId);
	});
}
 ...
 */
function processBlacklist(attributesToProcess, childsToTraverse, node, path, value, when, root) {
	...
	for (let idx = 0; idx < numberOfAttributes; idx++) {
		...
		if (when(constructPath(path, attr), node[attr], root)) {
