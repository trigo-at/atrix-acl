...
/* eslint no-underscore-dangle:0, no-prototype-builtins:0, max-len:0, dot-notation:0, no-restricted-syntax:0, no-labels:0, no-continue:0, no-restricted-globals:0 */
...
function tenantContextFilter({ path, value, contextTenantId, req, atrixACL }) {
	...
	if (value[atrixACL.config.acl.tenantIdProperty] === atrixACL.config.acl.matchAllTenantId || req.auth.tenantIds.includes(value[atrixACL.config.acl.tenantIdProperty])) return false;

	// match "_acl.tenantIds"
	if (value[atrixACL.config.acl.aclProperty] && intersection(value[atrixACL.config.acl.aclProperty].tenantIds && req.auth.tenantIds).length > 0) return false;
...
module.exports = (obj, filters, atrixACL, req, options) => {
	...
	if (options.filterTenantContext) {
		filters.splice(0, 0, {
			key: '*', value: undefined, when: tenantContextFilter,
		});
	}

	const whenClosure = (filter, path, value, root, contextTenantId) => (filter.when ? filter.when({ path, root, obj: value, value, filter, req, contextTenantId, atrixACL }) : true);
...
function filterNode(level, root, node, path, whenClosure, filterHatr, contextTenantId, filters, options) {
	...
	debug('filterNode:', level, root, /* node, */ path || '<ROOT>', /* whenClosure, filterHatr, */ contextTenantId, filters, options);
