...
module.exports = (obj, filter, req) => {
	...
	filterNode(obj, '', trie, filter.value, whenClosure);
...
function filterNode(node, path, filterTrie, value, when) {
	...
	childNodesToTraverse.forEach(attr => {
		...
		filterNode(nextNode, constructPath(path, attr), nextTrie, value, when);
	});
}
