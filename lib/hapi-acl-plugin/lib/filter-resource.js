...
module.exports = (obj, filters, shouldFilterHatrLinks, atrixACL, req) => {
	...
	const whenClosure = (filter, path, value, root) => (filter.when ? filter.when({ path, root, obj: value, value, filter, req }) : true);
...
function filterNode(level, root, node, path, filterTrie, when, filterHatr, contextTenantId) {
	...
	if (nodesToProcess.length > 0) {
		processBlacklist(root, node, path, nodesToProcess, childNodesToTraverse, filterTrie, when);
	}
 ...
 */
function processBlacklist(root, node, path, attributesToProcess, childsToTraverse, filterTrie, whenClosure) {
	...
	for (let idx = 0; idx < numberOfAttributes; idx++) {
		...
		filterDefinitions.forEach(filterDefinition => {
			if (whenClosure(filterDefinition, constructPath(path, attr), node[attr], root)) {
