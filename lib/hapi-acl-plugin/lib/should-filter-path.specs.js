...
describe('should-filter-path', () => {
	...
	it('should allow starts with search: "a.b.*"', () => {
		expect(shouldFilterPath('a.b', 'a.b.*')).to.be.false;
	...
	it('starts with search: "a.b.*" matches only sub properties', () => {
		expect(shouldFilterPath('a.b', 'a.b.*')).to.be.false;
	});

	it('starts with search: "a.b.*" matches array elements', () => {
		expect(shouldFilterPath('a.b', 'a.b.*', true)).to.be.true;
	});
	...
	it('should throw when path is null', () => {
		expect(() => shouldFilterPath(null, 'asd')).to.throw(/mandatory argument/);
	});
	it('should throw when path is undefined', () => {
		expect(() => shouldFilterPath(undefined, 'asd')).to.throw(/mandatory argument/);
	});
	it('should throw when keys is null', () => {
		expect(() => shouldFilterPath('a', null)).to.throw(/mandatory argument/);
	});
	it('should throw when keys is undefined', () => {
		expect(() => shouldFilterPath('a', undefined)).to.throw(/mandatory argument/);
	});
});
