...
const {
	isNil, type, map, pathOr, concat, intersection, or, pipe, isEmpty, not, filter, append,
} = require('ramda');
...
const hasAccess = (obj, tenantIds, roles, atrixACL) => {
	if (isNil(obj) || isNil(obj[atrixACL.config.acl.tenantIdProperty])) return true;

	const userRoles = concatRoles(roles);
	const userTenants = append(atrixACL.config.acl.matchAllTenantId, tenantIds);
	const matchTenants = concat(pathOr([], [atrixACL.config.acl.aclProperty, 'tenantIds'], obj), [obj[atrixACL.config.acl.tenantIdProperty]]);
	const matchRoles = pathOr([], [atrixACL.config.acl.aclProperty, 'roles'], obj);

	return or(
		isNotEmpty(intersection(matchTenants, userTenants)),
