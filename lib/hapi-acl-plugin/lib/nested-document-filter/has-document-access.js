...
const {
	has, contains, prop, pathOr, append, isNil, type,
} = require('ramda');
...
module.exports = (doc, userRoles, filteredUserTenants, atrixACL) => {
	...
	const hasTenantIdProperty = has(atrixACL.config.acl.tenantIdProperty);
	const hasTenantAclProperty = has(atrixACL.config.acl.aclProperty);
