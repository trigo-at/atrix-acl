...
module.exports = (req, atrixACL) => {
	const path = [
		...
		'credentials',
		'resource_access',
		atrixACL.tokenResourceAccessRoleKey,
		'roles',
	];
	...
	let roles = uniq(
		rawRoles
			.map(r => {
				...
				if (parts.length === 1)
					return {
						tenant: atrixACL.tokenResourceAccessRoleKey,
						role: r,
						global: true,
					};
				return apps.indexOf(parts[0]) !== -1
					? {tenant: parts[0], role: parts[1], global: false}
					: undefined;
			})
			.filter(r => !!r)
	);
	...
	const allTenantIds = pipe(concat(parsedACLs.tenantIds), uniq)(tenantIds);

	const allRoles = pipe(concat(parsedACLs.roles), uniq)(roles);
