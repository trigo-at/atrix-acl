...
const parseAclNoInjectBypassHeader = require('./handlers/parse-acl-no-inject-bypass-header');
...
module.exports = _atrix => {
    ...
    const aclPlugin = {
        register: (server, options, next) => {
            server.ext('onRequest', parseAclNoInjectBypassHeader(atrix));
            ...
            server.decorate('request', 'assertAccess', assignAssertAccess(atrix));
            next();
        },
    };
