...
module.exports = (_atrix) => {
	...
	const filterResponseBody = (req, next) => {
		...
		const filtered = atrix.ACL.filterRules.reduce((ret, filter) => filterResource(filter, ret), req.response.source);
	...
	const aclPlugin = {
		register: (server, options, next) => {
			...
			server.ext('onPreResponse', filterResponseBody);
			next();
		},
	};
