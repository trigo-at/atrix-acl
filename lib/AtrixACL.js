...
const configSchema = Joi.object({
	...
	tokenUsernameKey: Joi.string().default('preferred_username')
		.description('the key in the token tht contains the username. Value is attached to `req.auth.username`'),
	tokenUsernIdKey: Joi.string().default('userId')
		.description('the key in the token tht contains the userId. Value is attached to `req.auth.userId`'),
	enableNestedDocumentAcls: Joi.boolean().default(false)
		.description('wheter subdocuments are parsed for tenant scoped subdocuemtns (using: "tenantId" and "_acl" property)'),
	...
	requireAllACLs: Joi.boolean().default(false).description('When set the plugion fetchel all ACLs from server not just those scoped bz service name'),
