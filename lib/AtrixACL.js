...
const configSchema = Joi.object({
	...
	filterPropertiesDefinition: Joi.string().description(
		'path to the filter properties definition file'
	),
	filterPayloadDefinition: Joi.string().description(
		'path to filter payload definitions file'
	),
	...
	tokenResourceAccessRoleKey: Joi.string()
		...
		.description(
			'name of the default token ressource_access.<key> to get list of user roles'
		),
	...
	entityACLServiceUrl: Joi.string()
		.allow(null, '')
		.description(
			'the URL to ftech the entityACLs from. must provide the endpoint GET /acls/{serviceName}'
		),
	aclFetchInterval: Joi.number()
		.integer()
		.default(3000)
		.description(
			'Number of miliseconds to wait between entityACL refresh calls'
		),
});
...
const ruleSchema = Joi.object({
	...
	idParam: Joi.string().description(
		"the param name of the entities's id value in the path"
	),
});
...
class AtrixACL {
	constructor(atrix, service) {
		...
		if (!this.config.acl) {
			this.log.warn(
				`No "acl" section found config of service "${
					this.service.name
				}"`
			);
			return;
		...
		this.tenantIdsHeaderField =
			this.config.acl.tenantIdsHeaderField || 'x-pathfinder-tenant-ids';
	...
	setEntityACLDefinition(definitions) {
		if (this.ACL) {
			this.ACL.entityACLsDefinition = definitions;
		}
	}
	setEntityACLs(acls) {
		if (this.ACL) {
	...
	loadACLDefintion() {
		if (!fs.existsSync(this.config.acl.aclDefinition)) {
			throw new Error(
				`No aclDefinition found at "${this.config.acl.aclDefinition}"`
			);
		}
	...
	loadFilterPropertiesDefinition() {
		if (
			this.config.acl.filterPropertiesDefinition &&
			!fs.existsSync(this.config.acl.filterPropertiesDefinition)
		) {
			throw new Error(
				`No filterPropertiesDefinition found at "${
					this.config.acl.filterPropertiesDefinition
				}"`
			);
		}
		if (fs.existsSync(this.config.acl.filterPropertiesDefinition)) {
			const filterRules = require(this.config.acl
				.filterPropertiesDefinition)(); //eslint-disable-line
	...
	loadFilterPayloadDefinition() {
		if (
			this.config.acl.filterPayloadDefinition &&
			!fs.existsSync(this.config.acl.filterPayloadDefinition)
		) {
			throw new Error(
				`No filterPayloadDefinition found at "${
					this.config.acl.filterPayloadDefinition
				}"`
			);
		}
		if (fs.existsSync(this.config.acl.filterPayloadDefinition)) {
			const filterRules = require(this.config.acl
				.filterPayloadDefinition)(); //eslint-disable-line
	...
	setupEntityACLFetcher() {
		...
		const fetchAndLoadACLs = async () => {
			this.log.debug(
				`Fetching ACLs from: ${
					this.config.acl.entityACLServiceUrl
				}/acls/${this.service.name}...`
			);
	...
	getPrefix() {
		if (
			...
			this.config.endpoints.http &&
			this.config.endpoints.http.prefix
		) {
