...
const configSchema = Joi.object({
    ...
    allowInject: Joi.boolean()
        .default(true)
        .description('wheter to allow inject calls'),
    tokenResourceAccessRoleKey: Joi.string()
        .default('pathfinder-app')
        .description('name of the default token ressource_access.<key> to get list of user roles'),
    tokenUsernameKey: Joi.string()
        .default('preferred_username')
        .description('the key in the token tht contains the username. Value is attached to `req.auth.username`'),
    tokenUsernIdKey: Joi.string()
        .default('userId')
        .description('the key in the token tht contains the userId. Value is attached to `req.auth.userId`'),
    enableNestedDocumentAcls: Joi.boolean()
        .default(false)
        .description(
            'wheter subdocuments are parsed for tenant scoped subdocuemtns (using: "tenantId" and "_acl" property)'
        ),
    tenantIdsHeaderField: Joi.string()
        .default('x-pathfinder-tenant-ids')
        .description('name of the tenant ids header field'),
    tenantIdProperty: Joi.string()
        .default('tenantId')
        .description('name of the tenantId field for object filtering'),
    matchAllTenantId: Joi.string()
        .default('_all')
        .description('The magic tenant id value that matches all tenants'),
    aclProperty: Joi.string()
        .default('_acl')
        .description('name of the ACL property for object filtering'),
    endpoints: Joi.array()
        .items(Joi.string())
        .default([])
        .description('List of endpoint to apply ACLs on'),
    entityACLServiceUrl: Joi.string()
        .allow(null, '')
        .description('the URL to ftech the entityACLs from. must provide the endpoint GET /acls/{serviceName}'),
    requireAllACLs: Joi.boolean()
        .default(false)
        .description('When set the plugion fetchel all ACLs from server not just those scoped bz service name'),
    aclFetchInterval: Joi.number()
        .integer()
        .default(3000)
        .description('Number of miliseconds to wait between entityACL refresh calls'),
});

const stringOrStringArray = Joi.alternatives().try(Joi.string(), Joi.array().items(Joi.string()));
...
class AtrixACL {
    constructor(atrix, service) {
        ...
        if (!httpEndpoint.instance.server.registrations.AtrixACL) {
            httpEndpoint.instance.server.register(hapiACLPlugin(this), err => {
                //eslint-disable-line
                if (err) {
    ...
    async fetchAndLoadACLs() {
        if (!this.config.acl.entityACLServiceUrl) return;

        const fetchPath = this.config.acl.requireAllACLs
            ? `${this.config.acl.entityACLServiceUrl}/acls`
            : `${this.config.acl.entityACLServiceUrl}/acls/${this.service.name}`;

        this.log.debug(`Fetching ACLs from: ${fetchPath}...`);
        const res = await fetch(fetchPath, {headers: {'If-None-Match': this.etag}});
        if (res.status === 304) {
            this.log.debug('ACL cache hit');
            return;
        }
        if (res.status !== 200) {
            const body = await res.text();
            this.log.error(
                `Could not fetch ACLs from: ${this.config.acl.entityACLServiceUrl}/acls/${this.service.name}`,
                res.statusText,
                res.status,
                body
            );
    ...
    fixPath(path) {
        if (!path) throw new Error('mandatory argument "path" missing or empty');
