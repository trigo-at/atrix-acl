...
const configSchema = Joi.object({
	...
	entityACLServiceUrl: Joi.string().description('the URL to ftech the entityACLs from. must provide the endpoint GET /acls/{serviceName}'),
});
...
class AtrixACL {
	...
	loadEntityACLDefinition() {
		if (this.config.acl.entityACLsDefinition && !fs.existsSync(this.config.acl.entityACLsDefinition)) {
			throw new Error(`No entityACLsDefinition found at "${this.config.acl.entityACLsDefinition}"`);
		}
		if (fs.existsSync(this.config.acl.entityACLsDefinition)) {
			const filterRules = require(this.config.acl.entityACLsDefinition)(); //eslint-disable-line
			this.log.info('Loading entityACLsDefinition:', filterRules);
			this.ACL.entityACLsDefinition = filterRules;
		} else {
			this.log.info('No entityACLsDefinition configured.');
			this.ACL.entityACLsDefinition = [];
		}
	}
	...
	setupEntityACLFetcher() {
		...
		const fetchAndLoadACLs = async () => {
			this.log.info(`Fetching ACLs from: ${this.config.acl.entityACLServiceUrl}/acls/${this.serivce.name}...`);
			const res = await fetch(`${this.config.acl.entityACLServiceUrl}/acls/${this.serivce.name}`);
			if (res.status !== 200) {
				const body = await res.text();
				this.log.error(`Could not fetch ACLs from: ${this.config.acl.entityACLServiceUrl}/acls/${this.serivce.name}`, res.statusText, res.status, body);
				return;
			}
