...
describe('AtrixACL', () => {
	describe('constructor', () => {
		...
		it('checks if configured "filterPropertiesDefinition" exists & returns an array', () => {
			const atrix = new Atrix();
			const serivce = new atrix.Service('s', { endpoints, acl: { aclDefinition: path.join(__dirname, '../specs/acls'), filterPropertiesDefinition: 'foo-bar' } });
			serivce.endpoints.add('http');
			expect(() => new AtrixACL(atrix, serivce)).to.throw(Error);
			expect(() => new AtrixACL(atrix, serivce)).to.throw('No filterPropertiesDefinition found at "foo-bar"');
		});

		it('checks if configured "filterPayloadDefinition" exists & returns an array', () => {
			const atrix = new Atrix();
			const serivce = new atrix.Service('s', { endpoints, acl: { aclDefinition: path.join(__dirname, '../specs/acls'), filterPayloadDefinition: 'foo-bar' } });
			serivce.endpoints.add('http');
			expect(() => new AtrixACL(atrix, serivce)).to.throw(Error);
			expect(() => new AtrixACL(atrix, serivce)).to.throw('No filterPayloadDefinition found at "foo-bar"');
		});

		it('checks if configured "entityACLsDefinition" exists & returns an array', () => {
			const atrix = new Atrix();
			const serivce = new atrix.Service('s', { endpoints, acl: { aclDefinition: path.join(__dirname, '../specs/acls'), entityACLsDefinition: 'foo-bar' } });
			serivce.endpoints.add('http');
			expect(() => new AtrixACL(atrix, serivce)).to.throw(Error);
			expect(() => new AtrixACL(atrix, serivce)).to.throw('No entityACLsDefinition found at "foo-bar"');
		});
	...
	describe('congiuration file loading', () => {
		...
		it('loads and parses the "filterRules" definition into "ACL" instance', async () => {
			...
			const serivce = new atrix.Service('s', {
				...
				acl: {
					aclDefinition: path.join(__dirname, '../specs/acls'),
					filterPropertiesDefinition: path.join(__dirname, '../specs/filter-properties-rules.js'),
				},
			});
			serivce.endpoints.add('http');
			const as = new AtrixACL(atrix, serivce);
		...
		it('loads and parses the "entityACLsDefinition" definition into "ACL" instance', async () => {
			const atrix = new Atrix();
			const serivce = new atrix.Service('s', {
				endpoints,
				acl: {
					aclDefinition: path.join(__dirname, '../specs/acls'),
					entityACLsDefinition: path.join(__dirname, '../specs/entity-acl-config.js'),
				},
			});
			serivce.endpoints.add('http');
			const as = new AtrixACL(atrix, serivce);
			// await as.loadACLDefintion();
			expect(as.ACL).to.exist;
			expect(as.ACL.entityACLsDefinition).not.to.be.empty;
		});
	});
