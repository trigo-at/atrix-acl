...
describe('AtrixACL', () => {
	describe('constructor', () => {
		...
		it('checks if configure "acl -> aclDefinition" exist', () => {
			const atrix = new Atrix();
			atrix.configure({ pluginMap: { acl: path.join(__dirname, '../') } });
			const serivce = atrix.addService({ name: 'svc', endpoints, acl: { aclDefinition: 'gibts ned' } });
		...
		it('checks if configured "filterPropertiesDefinition" exists & returns an array', () => {
			const atrix = new Atrix();
			atrix.configure({ pluginMap: { acl: path.join(__dirname, '../') } });
			const serivce = atrix.addService({ name: 'svc', endpoints, acl: { aclDefinition: path.join(__dirname, '../specs/acls'), filterPropertiesDefinition: 'foo-bar' } });
			expect(() => new AtrixACL(atrix, serivce)).to.throw(Error);
		...
		it('checks if configured "filterPayloadDefinition" exists & returns an array', () => {
			const atrix = new Atrix();
			atrix.configure({ pluginMap: { acl: path.join(__dirname, '../') } });
			const serivce = atrix.addService({ name: 'svc', endpoints, acl: { aclDefinition: path.join(__dirname, '../specs/acls'), filterPayloadDefinition: 'foo-bar' } });
			expect(() => new AtrixACL(atrix, serivce)).to.throw(Error);
		...
		it('checks if http endpoint is registered', () => {
			const atrix = new Atrix();
			atrix.configure({ pluginMap: { acl: path.join(__dirname, '../') } });
			const serivce = atrix.addService({ name: 'svc', acl: { aclDefinition: 'gibts ned' } });
			expect(() => new AtrixACL(atrix, serivce)).to.throw(Error);
		...
		it('checks if allowInject config exists', () => {
			const atrix = new Atrix();
			atrix.configure({ pluginMap: { acl: path.join(__dirname, '../') } });
			const serivce = atrix.addService({ name: 'svc', endpoints, acl: { allowInject: false, aclDefinition: path.join(__dirname, '../specs/acls') } });
		...
		it('handles missing "entityACLServiceUrl" correctly', async () => {
			const atrix = new Atrix();
			atrix.configure({ pluginMap: { acl: path.join(__dirname, '../') } });
			const serivce = atrix.addService({
				name: 'svc',
	...
	describe('congiuration file loading', () => {
		it('loads and parses the "rules" definition into "ACL" instance', async () => {
			...
			expect(as.ACL.rules).to.eql(map(t => merge({ idParam: 'id' }, t), testRules));
		});
