...
describe('Response Filters', () => {
	...
	it('NOT filter wildcard properties in deeply nested objects (recursively)', async () => {
		...
		expect(res.body._embedded.event.legacy.courseRemark).to.equal(
			'legacy comment'
		);
	});
	...
	it('should filter sub-objects', async () => {
		atrixACL.setFilterRules([
			{
				key: '_embedded.*',
				when: ({root, value}) =>
					value.tenantId && value.tenantId !== root.tenantId,
				value: undefined,
			},
		]);
		...
		if (res.body._embedded.food && res.body._embedded.food.tenantId) {
			expect(res.body._embedded.food.tenantId).to.equal(
				res.body.tenantId
			);
		}
	});
