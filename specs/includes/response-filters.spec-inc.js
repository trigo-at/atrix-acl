...
describe('Response Filters', () => {
    ...
    it('apply filter if no "when" callback is set', async () => {
        atrixACL.setFilterRules([{key: '*.id', value: 'buh'}]);
    ...
    it('filter wildcard properties (recursively)', async () => {
        atrixACL.setFilterRules([{key: '*.id', when: () => true, value: 'buh'}]);
    ...
    it('filter wildcard properties in deeply nested objects (recursively)', async () => {
        atrixACL.setFilterRules([{key: ['*.legacy', '*.legacy_courseId'], when: () => true, value: undefined}]);

        const res = await svc.test.get('/prefix/attendee/43443209-7408-4df9-ba56-a3ff7509b699').set(testHeaders);
        expect(res.statusCode).to.equal(200);
    ...
    it('NOT filter wildcard properties in deeply nested objects (recursively)', async () => {
        atrixACL.setFilterRules([{key: ['*.legacy', '*.legacy_courseId'], when: () => false, value: undefined}]);
    ...
    it('should filter multiple keys', async () => {
        atrixACL.setFilterRules([{key: ['name', 'id'], when: () => true, value: 'buh'}]);
    ...
    it('should ignore properties not present in object', async () => {
        atrixACL.setFilterRules([{key: ['name', 'bla', 'bla.*.foo'], when: () => true, value: 'buh'}]);
    ...
    describe('consider methods', () => {
        it('should only apply filter for specific methods', async () => {
            ...
            let res = await svc.test.get('/prefix/pets/242').set(testHeaders);
            expect(res.statusCode).to.equal(200);
            expect(res.body.name).to.equal('buh');
            expect(res.body.id).to.not.equal('buh');

            res = await svc.test.post('/prefix/pets/242').set(testHeaders);
            expect(res.statusCode).to.equal(200);
    ...
    describe('consider roles', () => {
        ...
        it('role: should consider the effective roles of the current user (through token & tenant-id header)', async () => {
            ...
            let res = await svc.test.get('/prefix/pets/242').set(headers);
            expect(res.statusCode).to.equal(200);
            expect(res.body.name).to.equal('buh');
            ...
            res = await svc.test.get('/prefix/pets/242').set(headers);
            expect(res.statusCode).to.equal(200);
