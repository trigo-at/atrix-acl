...
describe('Entity ACLs', () => {
	...
	describe('extending route access', () => {
		it('ACLs with only tenantId copies entityTenantId scoped roles to the schared tenantId scope: "vida:admin" -> "gpa:admin"', async () => {
			atrixACL.setRules([{ tenant: 'gpa', role: 'admin', method: '*', entity: 'event', path: '/events/:id(/*_)', idParam: 'id' }]);
			atrixACL.setEntityACLs([{
				entity: 'event',
				id: '42',
				entityTenantId: 'gpa',
		...
		it('ACLs with role restrictions only copiy entityTenantId scoped roles to the shared tenantId scope when it exists in acl.roles and acl.tenantId scope user roles', async () => {
			atrixACL.setRules([{ tenant: 'gpa', role: 'admin', method: '*', entity: 'event', path: '/events/:id(/*_)', idParam: 'id' }]);
			atrixACL.setRules([{ tenant: 'gpa', role: 'viewer', method: '*', entity: 'budget', path: '/events/:eventId/budget/:id', idParam: 'id' }]);
			atrixACL.setEntityACLs([{
				entity: 'event',
				id: '42',
				entityTenantId: 'gpa',
			...
			expect((await svc.test.get('/prefix/events/42').set(getHeaders(['vida:admin', 'vida:viewer'], ['vida']))).statusCode).to.equal(401);
			expect((await svc.test.get('/prefix/events/42/budget/42').set(getHeaders(['vida:admin', 'vida:viewer'], ['vida']))).statusCode).to.equal(200);
		});
		...
		it('All matching ACLs are aplied to the users request context', async () => {
			...
			expect((await svc.test.get('/prefix/events/42').set(getHeaders(['vida:admin'], ['vida']))).statusCode).to.equal(200);
			expect((await svc.test.get('/prefix/events/42').set(getHeaders(['goed:admin'], ['goed']))).statusCode).to.equal(200);
		});
		...
		it('correctly matches rule.entity config', async () => {
			...
			expect((await svc.test.get('/prefix/events/42').set(getHeaders(['vida:admin'], ['vida']))).statusCode).to.equal(401);
		});

		it('correctly matches rule.idParam config', async () => {
			atrixACL.setRules([{ tenant: 'gpa', role: 'admin', method: '*', entity: 'event', path: '/events/:eventId(/*_)', idParam: 'eventId' }]);
			atrixACL.setEntityACLs([{
				entity: 'event',
				id: '42',
				entityTenantId: 'gpa',
		...
		it('correctly matches ACL id value config', async () => {
			...
			expect((await svc.test.get('/prefix/events/42').set(getHeaders(['vida:admin'], ['vida']))).statusCode).to.equal(200);
			expect((await svc.test.get('/prefix/events/43').set(getHeaders(['vida:admin'], ['vida']))).statusCode).to.equal(401);
		});
