...
describe('Filter endpoints', () => {
    ...
    describe('method:*, id:*', () => {
        ...
        it('allows POST', async () => {
            const res = await svc.test.post('/prefix/pets/242').set(testHeaders);
            expect(res.statusCode).to.equal(200);
        });

        it('allow random ID', async () => {
            const res = await svc.test.post('/prefix/pets/1234234').set(testHeaders);
            expect(res.statusCode).to.equal(200);
        });
    });
    ...
    describe('method:PUT ID:242 subResource:*', () => {
        ...
        it('allows PUT to wildcard sub-resources & action', async () => {
            const res = await svc.test.put('/prefix/pets/242/toys/bla/buy').set(testHeaders);
            expect(res.statusCode).to.equal(200);
        });

        it('allows PUT to wildcard sub-resources', async () => {
            const res = await svc.test.put('/prefix/pets/242/toys/bla').set(testHeaders);
            expect(res.statusCode).to.equal(200);
        });

        it('allows PUT to wildcard sub-resource', async () => {
            const res = await svc.test.put('/prefix/pets/242/toys').set(testHeaders);
            expect(res.statusCode).to.equal(200);
        });

        it('denies PUT to wildcard sub-resource with wrong ID', async () => {
            const res = await svc.test.put('/prefix/pets/123/toys').set(testHeaders);
            expect(res.statusCode).to.equal(401);
        });

        it('denies GET to wildcard sub-resource with wrong method', async () => {
            const res = await svc.test.get('/prefix/pets/123/toys').set(testHeaders);
            expect(res.statusCode).to.equal(401);
        });
    });
    describe('wildcard param in accessed path', () => {
        ...
        it('allows PUT to wildcard path (used in HATR links)', async () => {
            const res = await svc.test.put('/prefix/pets/242/toys/{toyId}').set(testHeaders);
            expect(res.statusCode).to.equal(200);
        });
    });
    describe('method:PUT ID:* subId:242', () => {
        ...
        it('allows PUT to specific sub-resources with wildcard main-resource', async () => {
            const res = await svc.test.put('/prefix/pets/242/toys/242').set(testHeaders);
            expect(res.statusCode).to.equal(200);
        });

        it('allows PUT to specific sub-resources with wildcard main-resource', async () => {
            const res = await svc.test.put('/prefix/pets/123/toys/242').set(testHeaders);
            expect(res.statusCode).to.equal(200);
        });

        it('denies PUT to wrong sub-resources with wildcard main-resource', async () => {
            const res = await svc.test.put('/prefix/pets/123/toys/123').set(testHeaders);
            expect(res.statusCode).to.equal(401);
        });

        it('denies PUT to specific sub-resources action with wildcard main-resource', async () => {
            const res = await svc.test.put('/prefix/pets/123/toys/242/buy').set(testHeaders);
            expect(res.statusCode).to.equal(401);
        });
    });
    describe('method:[GET, PUT] ID:* subId:242', () => {
        ...
        it('allows PUT to specific sub-resources with wildcard main-resource', async () => {
            const res = await svc.test.get('/prefix/pets/242/toys/242').set(testHeaders);
            expect(res.statusCode).to.equal(200);
        });

        it('allows PUT to specific sub-resources with wildcard main-resource', async () => {
            const res = await svc.test.get('/prefix/pets/123/toys/242').set(testHeaders);
            expect(res.statusCode).to.equal(200);
        });

        it('denies PUT to wrong sub-resources with wildcard main-resource', async () => {
            const res = await svc.test.post('/prefix/pets/123/toys/242').set(testHeaders);
            expect(res.statusCode).to.equal(401);
        });

        it('denies PUT to specific sub-resources action with wildcard main-resource', async () => {
            const res = await svc.test.post('/prefix/pets/242/toys/242').set(testHeaders);
            expect(res.statusCode).to.equal(401);
        });
